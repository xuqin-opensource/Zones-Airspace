...
class MyDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        """
        done标志位：表示episode结束的所有情况，包括：
        UAV死亡（碰撞、越界等）
        UAV成功到达目标
        达到环境最大步数限制
        dw标志位：仅表示真正的"死亡或胜利"情况，不包括时间截断

        在GAE（广义优势估计）计算中，两个标志位有不同作用：
        TD目标计算使用dw，优势计算使用done
        这种区分的理论基础是：
        时间截断（达到最大步数）不应该影响价值函数估计，因为UAV的状态仍然有价值
        真实结束（死亡或胜利）时，后续价值为0，需要正确处理bootstrap
        """
        return {
...
class PPO(object):
    def __init__(
            ...
            gamma=0.99,
            lambd=0.95,     # GAE（广义优势估计）的λ参数
            # gamma=0.89,
            # lambd=0.88,
            clip_rate=0.2,  # PPO剪裁率，通常为0.2
            K_epochs=10,    # 每次更新的epoch数
            net_width=256,  # 网络宽度（特征维度）
            a_lr=3e-4,      # Actor网络学习率
            c_lr=3e-4,      # Critic网络学习率
            l2_reg=1e-3,    # L2 regulization for Critic
            dist='Beta',    # 动作分布类型
            a_optim_batch_size=64,  # Actor优化批量大小
            c_optim_batch_size=64,  # Critic优化批量大小
            entropy_coef=0,         # 熵系数，鼓励探索
            entropy_coef_decay=0.9998,  # 熵系数衰减
            writer=None,        # TensorBoard写入器
            activation=None,    # 激活函数
            share_layer_flag=True,  # actor和critic是否共享权重
            anneal_lr=True,     # 是否退火学习率
            totoal_steps=0,     # 总训练步数
            with_position=False,    # 是否包含位置信息
            token_query=False,      # 是否使用token查询
            num_enc=5,      # 编码器数量
            logger=None,    # 日志记录器
            dir=None,       # 保存目录
            test=False,
            net_model='fc1',    # 网络模型类型
            beta_base=1e-5  # Beta分布基础参数
    ):
        # if net_model == 'fc':
        ...
        self.dir = dir
        self.logger = logger
        self.share_layer_flag = share_layer_flag    # actor和critic是否共享权重
    ...
    def select_action(self, s1, s2):  # only used when interact with the env
        ...
        with torch.no_grad():
            ...
            dist, alpha, beta, nan_event = self.actor.get_dist(s1, s2, self.logger) # 评估时使用众数而非采样
            ...
            if nan_event:
                self.save('nan')
                sys.exit()
            a = dist.sample()   # 从分布中采样动作
            assert torch.all((0 <= a)) and torch.all(a <= 1)
            a = torch.clamp(a, 0, 1)    # 将张量a中每个元素的值限制在 [0, 1] 范围内。
            logprob_a = dist.log_prob(a).cpu().numpy()  # 计算动作的对数概率
    ...
    def train(self, global_step, epoches=None):
        ...
        transitions = self.gae()    # 计算GAE，transitions=(s1, s2, a, r, s1', s2', logprob_a, done, dw, td_target, adv_normal)
        dataset = MyDataset(transitions)    # 创建数据加载器

        dataloader = DataLoader(dataset, batch_size=self.a_optim_batch_size, shuffle=True, drop_last=True)  # 多轮训练，每次使用小批量数据
        ...
        for i in range(epoches):
            ...
            for batch in dataloader:
                ...
                distribution, _, _, nan_event = self.actor.get_dist(s1, s2, self.logger)    # distribution为动作的概率分布且符合Beta分布
                if nan_event:
                    self.save('nan')    # 保存模型，NaN通常意味着数值不稳定或梯度爆炸
                # 计算分布熵（-Σplogp），sum(1, keepdim=True)：对动作维度求和（多维度动作），熵越大表示探索性越强
                dist_entropy = distribution.entropy().sum(1, keepdim=True)
                logprob_a_now = distribution.log_prob(a)    # 计算当前策略下采取动作a的对数概率

                logratio = logprob_a_now.sum(1, keepdim=True) - logprob_a.sum(1, keepdim=True)  # 计算新旧策略的对数概率比
                ratio = torch.exp(logratio)  # 计算概率比：a/b == exp(log(a)-log(b))

                with torch.no_grad():
                    # calculate approx_kl http://joschu.net/blog/kl-approx.html
                    old_approx_kl = (-logratio).mean()
                    approx_kl = ((ratio - 1) - logratio).mean()
                    # 计算裁剪比例；统计有多少比例的ratio超出了裁剪范围[1-clip_rate, 1+clip_rate]，用于监控裁剪是否频繁发生
                    clipfracs += [torch.mean((ratio - 1.0).abs() > self.clip_rate, dtype=torch.float32).item()]

                surr1 = ratio * adv # 原始的策略梯度目标：r(θ) * A，维度为: (batch_size, 1)
                surr2 = torch.clamp(ratio, 1 - self.clip_rate, 1 + self.clip_rate) * adv    # 裁剪后的替代值，将ratio限制在[1-ε, 1+ε]范围内，维度为: (batch_size, 1)
                pg_loss = -torch.min(surr1, surr2)  # 负号表示最大化目标，维度为: (batch_size, 1)
                # Actor总损失 = 策略梯度损失 + 熵奖励，a_loss是一个向量，每个元素对应一个样本的损失。我们需要将其转换为标量才能反向传播
                # 熵奖励：鼓励探索，防止策略过早收敛到局部最优，entropy_coef会随时间衰减，维度为: (batch_size, 1)
                a_loss = pg_loss - self.entropy_coef * dist_entropy 

                

                '''updata parameters'''
                self.actor_optimizer.zero_grad()    # 清空Actor优化器的梯度，防止梯度累积
                # 反向传播计算Actor梯度，mean()：对批次取平均，retain_graph=self.share_layer_flag：如果共享层，保留计算图供Critic使用
                a_loss.mean().backward(retain_graph=self.share_layer_flag)
                torch.nn.utils.clip_grad_norm_(self.actor.parameters(), 40) # 梯度裁剪，将Actor参数的梯度范数限制在40以内
                ...
                # 计算Critic的MSE损失，L = (V_θ(s) - (V_old(s) + A))²，td_target = V_old(s) + A（来自GAE计算）
                c_loss = (self.critic(s1, s2) - td_target).pow(2).mean()
                for name, param in self.critic.named_parameters():  # named_parameters()返回模型中所有可训练参数的名称(name)和参数本身(param)
                    if 'weight' in name:    # 只对权重参数（weight）应用L2正则，不对偏置（bias）应用
                        c_loss += param.pow(2).sum() * self.l2_reg  # 对所有权重参数施加L2正则化惩罚，防止过拟合
                c_loss.backward()   # mean()已经在计算MSE时调用了
        ...
        # del vs, td_target
        del a_loss, c_loss, pg_loss, dist_entropy, old_approx_kl, approx_kl, logprob_a_now, logratio  # 释放计算图和梯度内存
        del surr1, surr2
    ...
    def gae(self, unification=True):
        transitions = []    # 存储处理后的所有转移数据

        collect_adv = []    # 收集所有优势值，用于后续归一化
        for agent in self.data: # self.data是一个字典，键是智能体ID，值是该智能体的经验轨迹
            s1, s2, _, r, s1_prime, s2_prime, _, done_mask, dw_mask = self.make_batch(agent)    # 从该智能体的经验中提取批量数据，忽略动作和对数概率

            ''' Use TD+GAE+LongTrajectory to compute Advantage and TD target'''
            self.critic.eval()
            with torch.no_grad():
                vs = self.critic(s1, s2)    # 当前状态的价值
                vs_ = self.critic(s1_prime, s2_prime)   # 下一状态的价值
                # 计算TD误差，(1 - dw_mask)：如果dw=True（死亡/终止），则未来价值为0；deltas维度：(batch_size, 1)
                deltas = r + self.gamma * vs_ * (1 - dw_mask) - vs
                deltas = deltas.cpu().flatten().numpy()
                adv = [0]   # 初始化优势列表，末尾添加一个0作为初始值
                # 反向递归计算GAE，A_t = δ_t + γ * λ * A_{t+1} * (1 - done_t)
                for dlt, mask in zip(deltas[::-1], done_mask.cpu().flatten().numpy()[::-1]):
                    advantage = dlt + self.gamma * self.lambd * adv[-1] * (1 - mask)
                    adv.append(advantage)
                adv.reverse()   # 反转回正向时间顺序
                adv = copy.deepcopy(adv[0:-1])  # 移除末尾添加的初始0
                collect_adv += adv
                # td_target：用于训练critic的目标值
                td_target = np.array(adv) + np.array(vs.to('cpu').squeeze(1))   # vs.squeeze(1)：从(batch_size, 1)变为(batch_size,)
            for i, single_transition in enumerate(self.data[agent]):
                # (s1, s2, a, r, s1', s2', logprob_a, done, dw, td_target, adv)
                transitions.append(single_transition + [[td_target[i]], adv[i]])
        adv_mean = np.mean(collect_adv) # 全局优势均值
        adv_std = np.std(collect_adv)   # 全局优势标准差
        # tran[0:-1]取前10个元素（排除最后一个adv），对最后一个元素adv进行归一化：advantage_normalized = (advantage - mean) / (std + 1e-6)
